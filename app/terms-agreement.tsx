...
function TermsAgreement() {
    ...
    (async () => {
      try {
        ...
        if (res.success && res.data) {
          ...
          // API failed — allow user to proceed anyway
          setIsScrolledToBottom(true);
        }
      } catch (e) {
  ...
  // Auto-enable agree button after content loads (short content case)
  useEffect(() => {
    if (!loading && content) {
      const timer = setTimeout(() => {
        if (!isScrolledToBottom) {
          // If content didn't trigger scroll (fits in viewport), enable after 1s
          setIsScrolledToBottom(true);
        }
      }, 1500);
  ...
  const handleScroll = (event: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    // Also enable if content fits within the viewport (no need to scroll)
